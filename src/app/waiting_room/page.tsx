...
export default function Home() {
  ...
  async function createRoom() {
    ...
    const response = await fetch('https://chatapp-backend-lhk3.onrender.com/api/create-room/', {
      method: 'POST',
  ...
  async function joinRoom(id: number) {
    ...
    const response = await fetch(`https://chatapp-backend-lhk3.onrender.com/api/join-room/${id}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    async function fetchRooms() {
      const response = await fetch('https://chatapp-backend-lhk3.onrender.com/api/list-rooms/');
      if (!response.ok) {
